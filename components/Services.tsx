...
const Services = memo(() => {
  ...
  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 mb-16">
    ...
    <motion.div
      ...
      whileHover={{ y: -5 }}
      className="relative p-6 md:p-8 rounded-lg bg-white border border-gray-200 hover:shadow-md transition-all duration-300"
    >
      <div className="w-12 h-12 rounded-full flex items-center justify-center mb-4 bg-gray-800">
        <Palette className="h-5 w-5 text-white" />
      </div>
      <div className="mb-4">
        <h3 className="text-xl font-serif font-medium mb-2 text-gray-800">
          Editing & Retouching
        </h3>
        <p className="text-sm mb-4 text-gray-600">
          Professional photo editing, retouching, and enhancement for all your images.
        </p>
      ...
      <div className="space-y-2 mb-6">
        {["Color correction", "Skin retouching", "Background removal", "Creative effects"].map((feature, featureIndex) => (
          <motion.div
            key={featureIndex}
            initial={{ opacity: 0, x: -10 }}
            animate={isInView ? { opacity: 1, x: 0 } : {}}
            transition={{
              duration: 0.4,
              delay: 0.4 + services.length * 0.1 + featureIndex * 0.05,
            }}
            className="flex items-start space-x-2"
          >
      ...
      <motion.a
        ...
        whileHover={{ scale: 1.02 }}
        whileTap={{ scale: 0.98 }}
        className="w-full flex items-center justify-center space-x-2 px-4 py-2 rounded text-sm font-medium bg-gray-800 text-white hover:bg-gray-700 transition-all duration-300"
      >
        <span>Call Now: +91 9336981925</span>
        <ArrowRight className="h-3 w-3" />
      </motion.a>
  ...
  <motion.div
        ...
        >
          ...
          <p className="text-sm md:text-base text-gray-300 mb-6 md:mb-8 max-w-2xl mx-auto">
            ...
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="px-6 py-3 bg-amber-600 text-white font-medium rounded hover:bg-amber-700 transition-colors duration-300 inline-block mb-4"
            >
            Call: +91 9336981925
            </motion.a>
            <motion.a
            href="mailto:goldenkasia@gmail.com"
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="px-6 py-3 bg-gray-800 text-white font-medium rounded hover:bg-gray-700 transition-colors duration-300 inline-block"
            >
            Email: goldenkasia@gmail.com
            </motion.a>
        </motion.div>
    </section>
  );
